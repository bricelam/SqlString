...
internal struct RoutePatternLexer
{
    ...
    internal RoutePatternToken? TryScanLiteral()
    {
        ...
        {
            token = token.AddDiagnosticIfNone(new EmbeddedDiagnostic(
                string.Format(Resources.TemplateRoute_InvalidLiteral, token.Value),
                token.GetSpan()));
        }
    ...
    internal RoutePatternToken? TryScanParameterName()
    {
        ...
        {
            token = token.AddDiagnosticIfNone(
                new EmbeddedDiagnostic(string.Format(Resources.TemplateRoute_InvalidParameterName, token.Value!.ToString().Replace("{{", "{").Replace("}}", "}")), token.GetSpan()));
        }
